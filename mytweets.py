...
def getTweets(num):
    request = ""
    if(num == -1):
        request = "https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=" + \
            sys.argv[1] + "&count=200&include_rts=1&trim_user=1"
    else:
        request = "https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=" + \
            sys.argv[1] + \
            "&count=200&include_rts=1&trim_user=1&since_id=" + num
...
def processTweets(obj):
    if(len(obj) == 200):
        ...
        if(tweet_id == -1):
            request = "https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=" + \
                sys.argv[1] + \
                "&count=200&include_rts=1&trim_user=1&max_id=" + str(maxid)
        else:
            request = "https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=" + \
                sys.argv[1] + "&count=200&include_rts=1&trim_user=1&max_id=" + \
                str(maxid) + "&since_id=" + tweet_id
